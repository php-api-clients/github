...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\TimelineReviewedEvent\Links;
...
final readonly class TimelineReviewedEvent
{
    ...
    public function __construct(string $event, int $id, string $node_id, SimpleUser $user, string $body, string $state, string $html_url, string $pull_request_url, Links $_links, string $submitted_at, string $commit_id, string $body_html, string $body_text, string $author_association)
    {
        $this->event              = $event;
        $this->id                 = $id;
