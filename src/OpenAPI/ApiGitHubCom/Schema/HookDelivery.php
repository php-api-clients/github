...
final readonly class HookDelivery
{
    ...
    public function __construct(int $id, string $guid, string $delivered_at, bool $redelivery, int $duration, string $status, int $status_code, string $event, string $action, int $installation_id, int $repository_id, string $url, Request $request, Response $response)
    {
        $this->id              = $id;
        $this->guid            = $guid;
