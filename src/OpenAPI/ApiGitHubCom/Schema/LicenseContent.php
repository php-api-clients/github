...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\ContentTree\Links;
...
final readonly class LicenseContent
{
    ...
    public function __construct(string $name, string $path, string $sha, int $size, string $url, string $html_url, string $git_url, string $download_url, string $type, string $content, string $encoding, Links $_links, mixed $license)
    {
        $this->name         = $name;
        $this->path         = $path;
        $this->sha          = $sha;
