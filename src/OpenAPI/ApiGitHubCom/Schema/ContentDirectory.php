...
final readonly class ContentDirectory
{
    ...
    public function __construct(string $type, int $size, string $name, string $path, string $content, string $sha, string $url, string $git_url, string $html_url, string $download_url, \ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\ContentTree\Links $_links)
    {
        $this->type = $type;
        $this->size = $size;
        $this->name = $name;
