...
final readonly class ContentSymlink
{
    ...
    public function __construct(string $type, string $target, int $size, string $name, string $path, string $sha, string $url, string $git_url, string $html_url, string $download_url, \ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\ContentTree\Links $_links)
    {
        $this->type = $type;
        $this->target = $target;
        $this->size = $size;
