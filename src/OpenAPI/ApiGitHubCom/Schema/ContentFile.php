...
final readonly class ContentFile
{
    ...
    public function __construct(string $type, string $encoding, int $size, string $name, string $path, string $content, string $sha, string $url, string $git_url, string $html_url, string $download_url, \ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\ContentTree\Links $_links, string $target, string $submodule_git_url)
    {
        $this->type = $type;
