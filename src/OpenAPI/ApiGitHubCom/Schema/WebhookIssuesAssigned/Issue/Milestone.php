...
final readonly class Milestone
{
    ...
    public ?string $url;
    public function __construct(string $closed_at, int $closed_issues, string $created_at, \ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\WebhookIssueCommentDeleted\Comment\User $creator, string $description, string $due_on, string $html_url, int $id, string $labels_url, string $node_id, int $number, int $open_issues, string $state, string $title, string $updated_at, string $url)
    {
