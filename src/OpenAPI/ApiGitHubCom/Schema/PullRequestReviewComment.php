...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\PullRequestReviewComment\Links;
...
final readonly class PullRequestReviewComment
{
    ...
    public function __construct(string $url, int $pull_request_review_id, int $id, string $node_id, string $diff_hunk, string $path, int $position, int $original_position, string $commit_id, string $original_commit_id, int $in_reply_to_id, SimpleUser $user, string $body, string $created_at, string $updated_at, string $html_url, string $pull_request_url, string $author_association, Links $_links, int $start_line, int $original_start_line, string $start_side, int $line, int $original_line, string $side, ReactionRollup $reactions, string $body_html, string $body_text)
    {
        $this->url                    = $url;
