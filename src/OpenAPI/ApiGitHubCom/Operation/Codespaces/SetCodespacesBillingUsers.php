...
{
    private const OPERATION_ID = 'codespaces/set-codespaces-billing-users';
    ...
    {
        $this->requestSchemaValidator->validate($data, \cebe\openapi\Reader::readFromJson(\ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\SetCodespacesBillingUsers\Request\Application\Json::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
        return new \RingCentral\Psr7\Request('POST', \str_replace(array('{org}'), array($this->org), '/orgs/{org}/codespaces/billing/selected_users'), array('Content-Type' => 'application/json'), json_encode($data));
    }
    /**
