...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Hydrator\Operation\Orgs\CbOrgRcb\Packages\CbPackageTypeRcb\CbPackageNameRcb\Versions;
...
final class GetAllPackageVersionsForPackageOwnedByOrg
{
    ...
    public function __construct(SchemaValidator $responseSchemaValidator, Versions $hydrator, string $package_type, string $package_name, string $org, int $page = 1, int $per_page = 30, string $state = 'active')
    {
    ...
    {
        return new Request(self::METHOD, str_replace(['{package_type}', '{package_name}', '{org}', '{page}', '{per_page}', '{state}'], [$this->package_type, $this->package_name, $this->org, $this->page, $this->per_page, $this->state], self::PATH . '?page={page}&per_page={per_page}&state={state}'));
    }
