...
{
    private const OPERATION_ID = 'actions/update-required-workflow';
    ...
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $requestSchemaValidator, \League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, \ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Hydrator $hydrator, string $org, int $required_workflow_id)
    {
    ...
    {
        $this->requestSchemaValidator->validate($data, \cebe\openapi\Reader::readFromJson(\ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\UpdateRequiredWorkflow\Request\Application\Json::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
        return new \RingCentral\Psr7\Request('PATCH', \str_replace(array('{org}', '{required_workflow_id}'), array($this->org, $this->required_workflow_id), '/orgs/{org}/actions/required_workflows/{required_workflow_id}'), array('Content-Type' => 'application/json'), json_encode($data));
    }
    /**
