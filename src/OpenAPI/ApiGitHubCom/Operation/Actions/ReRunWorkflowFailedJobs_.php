...
final class ReRunWorkflowFailedJobs_
{
    private const OPERATION_ID = 'actions/re-run-workflow-failed-jobs';
    ...
    {
        $this->requestSchemaValidator->validate($data, \cebe\openapi\Reader::readFromJson(\ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\Unknown\CD2A759130388E4E1Cc684Aa946754F39::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
        return new \RingCentral\Psr7\Request('post', \str_replace(array('{owner}', '{repo}', '{run_id}'), array($this->owner, $this->repo, $this->run_id), '/repos/{owner}/{repo}/actions/runs/{run_id}/rerun-failed-jobs'), array('Content-Type' => 'application/json'), json_encode($data));
    }
