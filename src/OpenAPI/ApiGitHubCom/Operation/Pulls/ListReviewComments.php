...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Hydrator\Operation\Repos\CbOwnerRcb\CbRepoRcb\Pulls\CbPullNumberRcb\Comments;
...
final class ListReviewComments
{
    ...
    public function __construct(SchemaValidator $responseSchemaValidator, Comments $hydrator, string $owner, string $repo, int $pull_number, string $direction, string $since, string $sort = 'created', int $per_page = 30, int $page = 1)
    {
        $this->owner                   = $owner;
        $this->repo                    = $repo;
    ...
    {
        return new Request(self::METHOD, str_replace(['{owner}', '{repo}', '{pull_number}', '{direction}', '{since}', '{sort}', '{per_page}', '{page}'], [$this->owner, $this->repo, $this->pull_number, $this->direction, $this->since, $this->sort, $this->per_page, $this->page], self::PATH . '?direction={direction}&since={since}&sort={sort}&per_page={per_page}&page={page}'));
    }
