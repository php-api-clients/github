...
final class ListDiscussionsInOrg
{
    ...
    public function __construct(SchemaValidator $responseSchemaValidator, Discussions $hydrator, string $org, string $team_slug, string $pinned, string $direction = 'desc', int $per_page = 30, int $page = 1)
    {
        $this->org                     = $org;
    ...
    {
        return new Request(self::METHOD, str_replace(['{org}', '{team_slug}', '{pinned}', '{direction}', '{per_page}', '{page}'], [$this->org, $this->team_slug, $this->pinned, $this->direction, $this->per_page, $this->page], self::PATH . '?pinned={pinned}&direction={direction}&per_page={per_page}&page={page}'));
    }
