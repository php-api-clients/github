...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Hydrator\Operation\Enterprises\CbEnterpriseRcb\Dependabot\Alerts;
...
final class ListAlertsForEnterprise
{
    ...
    public function __construct(SchemaValidator $responseSchemaValidator, Alerts $hydrator, string $enterprise, string $state, string $severity, string $ecosystem, string $package, string $scope, string $before, string $after, int $last, string $sort = 'created', string $direction = 'desc', int $first = 30, int $per_page = 30)
    {
    ...
    {
        return new Request(self::METHOD, str_replace(['{enterprise}', '{state}', '{severity}', '{ecosystem}', '{package}', '{scope}', '{before}', '{after}', '{last}', '{sort}', '{direction}', '{first}', '{per_page}'], [$this->enterprise, $this->state, $this->severity, $this->ecosystem, $this->package, $this->scope, $this->before, $this->after, $this->last, $this->sort, $this->direction, $this->first, $this->per_page], self::PATH . '?state={state}&severity={severity}&ecosystem={ecosystem}&package={package}&scope={scope}&before={before}&after={after}&last={last}&sort={sort}&direction={direction}&first={first}&per_page={per_page}'));
    }

    /**
     * @return Observable<DependabotAlertWithRepository>|BasicError|ValidationErrorSimple
     */
    function createResponse(ResponseInterface $response): Observable|BasicError|ValidationErrorSimple
    {
