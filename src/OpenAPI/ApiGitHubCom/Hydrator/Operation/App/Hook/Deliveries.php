...
use ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\HookDeliveryItem;
...
class Deliveries implements ObjectMapper
{
    ...
    private function hydrateApiClients⚡️Client⚡️Github⚡️OpenAPI⚡️ApiGitHubCom⚡️Schema⚡️HookDeliveryItem(array $payload): HookDeliveryItem
    {
        $properties    = [];
        $missingFields = [];
        try {
        ...
        if (count($missingFields) > 0) {
            throw UnableToHydrateObject::dueToMissingFields(HookDeliveryItem::class, $missingFields, stack: $this->hydrationStack);
        }

        try {
            return new HookDeliveryItem(...$properties);
        } catch (Throwable $exception) {
            throw UnableToHydrateObject::dueToError('ApiClients\Client\Github\OpenAPI\ApiGitHubCom\Schema\HookDeliveryItem', $exception, stack: $this->hydrationStack);
        }
    }
    ...
    {
        assert($object instanceof HookDeliveryItem);
        $result = [];
