...
final class ListAlertsForOrg
{
    ...
    private readonly \ApiClients\Client\Github\Hydrator\Operation\Orgs\CbOrgRcb\Dependabot\Alerts $hydrator;
    public function __construct(\League\OpenAPIValidation\Schema\SchemaValidator $responseSchemaValidator, \ApiClients\Client\Github\Hydrator\Operation\Orgs\CbOrgRcb\Dependabot\Alerts $hydrator, string $org, string $state, string $severity, string $ecosystem, string $package, string $scope, string $before, string $after, int $last, string $sort = 'created', string $direction = 'desc', int $first = 30, int $per_page = 30)
    {
        $this->org = $org;
        $this->state = $state;
    ...
    /**
     * @return \Rx\Observable<\ApiClients\Client\Github\Schema\DependabotAlertWithRepository>|\ApiClients\Client\Github\Schema\BasicError|\ApiClients\Client\Github\Schema\ScimError|\ApiClients\Client\Github\Schema\ValidationErrorSimple
     */
    function createResponse(\Psr\Http\Message\ResponseInterface $response) : \Rx\Observable|\ApiClients\Client\Github\Schema\BasicError|\ApiClients\Client\Github\Schema\ScimError|\ApiClients\Client\Github\Schema\ValidationErrorSimple
    {
            ...
            case 200:
                switch ($contentType) {
                    case 'application/json':
                        $this->responseSchemaValidator->validate($body, \cebe\openapi\Reader::readFromJson(\ApiClients\Client\Github\Schema\DependabotAlertWithRepository::SCHEMA_JSON, '\\cebe\\openapi\\spec\\Schema'));
                        return \Rx\Observable::fromArray($body, new \Rx\Scheduler\ImmediateScheduler())->map(function (array $body) : \ApiClients\Client\Github\Schema\DependabotAlertWithRepository {
                            return $this->hydrator->hydrateObject('\\ApiClients\\Client\\Github\\Schema\\DependabotAlertWithRepository', $body);
                        });
                }
                break;
            /**Validation failed, or the endpoint has been spammed.**/
