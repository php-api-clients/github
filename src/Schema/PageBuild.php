...
final readonly class PageBuild
{
    ...
    public function __construct(string $url, string $status, \ApiClients\Client\Github\Schema\PageBuild\Error $error, mixed $pusher, string $commit, int $duration, string $created_at, string $updated_at)
    {
        $this->url = $url;
        $this->status = $status;
        $this->error = $error;
