<?php

declare (strict_types=1);
namespace ApiClients\Client\Github\Schema\PullRequestMinimal;

final readonly class Head
{
    public const SCHEMA_JSON = '{"required":["ref","sha","repo"],"type":"object","properties":{"ref":{"type":"string"},"sha":{"type":"string"},"repo":{"required":["id","url","name"],"type":"object","properties":{"id":{"type":"integer"},"url":{"type":"string"},"name":{"type":"string"}}}}}';
    public const SCHEMA_TITLE = '';
    public const SCHEMA_DESCRIPTION = '';
    public ?string $ref;
    public ?string $sha;
    public ?\ApiClients\Client\Github\Schema\PullRequestMinimal\Head\Repo $repo;
    public function __construct(string $ref, string $sha, \ApiClients\Client\Github\Schema\PullRequestMinimal\Head\Repo $repo)
    {
        $this->ref = $ref;
        $this->sha = $sha;
        $this->repo = $repo;
    }
}
