...
final readonly class ContentTree
{
    ...
    public function __construct(string $type, int $size, string $name, string $path, string $sha, string $url, string $git_url, string $html_url, string $download_url, array $entries, \ApiClients\Client\Github\Schema\ContentTree\Links $_links)
    {
        $this->type = $type;
        $this->size = $size;
        $this->name = $name;
