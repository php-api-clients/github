...
final readonly class Page
{
    ...
    public \ApiClients\Client\Github\Schema\PagesSourceHash $source;
    /**
     * Whether the GitHub Pages site is publicly visible. If set to `true`, the site is accessible to anyone on the internet. If set to `false`, the site will only be accessible to users who have at least `read` access to the repository that published the site.
     */
    public ?bool $public;
    public \ApiClients\Client\Github\Schema\PagesHttpsCertificate $https_certificate;
    /**
     * Whether https is enabled on the domain
     */
    public bool $https_enforced;
    public function __construct(string $url, string $status, string $cname, string $protected_domain_state, string $pending_domain_unverified_at, bool $custom_404, string $html_url, string $build_type, \ApiClients\Client\Github\Schema\PagesSourceHash $source, bool $public, \ApiClients\Client\Github\Schema\PagesHttpsCertificate $https_certificate, bool $https_enforced)
    {
        $this->url = $url;
        $this->status = $status;
        $this->cname = $cname;
