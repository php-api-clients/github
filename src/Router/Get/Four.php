...
final class Four
{
    ...
    /** @return |iterable<Schema\HookDeliveryItem>|iterable<Schema\GistComment>|array{code:int}|iterable<Schema\GistCommit>|iterable<Schema\GistSimple>|Schema\GitignoreTemplate|iterable<Schema\MarketplaceListingPlan>|Schema\Thread|iterable<Schema\SimpleUser>|Schema\Operations\Codespaces\ListInOrganization\Response\ApplicationJson\Ok|iterable<Schema\Event>|iterable<Schema\OrganizationInvitation>|iterable<Schema\OrgHook>|Schema\InteractionLimitResponse|Schema\Operations\Interactions\GetRestrictionsForOrg\Response\ApplicationJson\Ok\Application\Json\One|iterable<Schema\Issue>|iterable<Schema\Migration>|iterable<Schema\Package>|iterable<Schema\OrganizationProgrammaticAccessGrantRequest>|iterable<Schema\OrganizationProgrammaticAccessGrant>|iterable<Schema\Project>|iterable<Schema\MinimalRepository>|iterable<Schema\RepositoryRuleset>|iterable<Schema\RepositoryAdvisory>|iterable<Schema\TeamSimple>|iterable<Schema\Team>|Schema\ProjectColumn|iterable<Schema\ProjectColumn>|iterable<Schema\TeamDiscussion>|iterable<Schema\TeamProject>|Schema\Codespace|Schema\GpgKey|Schema\Key|iterable<Schema\UserMarketplacePurchase>|iterable<Schema\OrgMembership>|Schema\Migration|Schema\SshSigningKey|iterable<Schema\BaseGist>|iterable<Schema\GpgKey>|iterable<Schema\KeySimple>|iterable<Schema\OrganizationSimple>|iterable<Schema\SocialAccount>|iterable<Schema\SshSigningKey>|Schema\StarredRepository|Schema\Repository */
                ...
                if ($pathChunks[2] === 'accounts') {
                    if ($pathChunks[3] === '{account_id}') {
                        if ($call === 'GET /marketplace_listing/accounts/{account_id}') {
                            return $this->routers->router🔀Get🔀Apps()->getSubscriptionPlanForAccount($params);
                        }
                    }
                } elseif ($pathChunks[2] === 'stubbed') {
                    if ($pathChunks[3] === 'plans') {
                        if ($call === 'GET /marketplace_listing/stubbed/plans') {
                            return $this->routers->router🔀Get🔀Apps()->listPlansStubbed($params);
                        }
                    }
                }
                ...
                if ($pathChunks[2] === '{org}') {
                    if ($pathChunks[3] === 'blocks') {
                        if ($call === 'GET /orgs/{org}/blocks') {
                            return $this->routers->router🔀Get🔀Orgs()->listBlockedUsers($params);
                        }
                    } elseif ($pathChunks[3] === 'codespaces') {
                        if ($call === 'GET /orgs/{org}/codespaces') {
                            return $this->routers->router🔀Get🔀Codespaces()->listInOrganization($params);
                        }
                        ...
                        if ($call === 'GET /orgs/{org}/failed_invitations') {
                            return $this->routers->router🔀Get🔀Orgs()->listFailedInvitations($params);
                        }
                        ...
                        if ($call === 'GET /orgs/{org}/interaction-limits') {
                            return $this->routers->router🔀Get🔀Interactions()->getRestrictionsForOrg($params);
                        }
                    } elseif ($pathChunks[3] === 'invitations') {
                        if ($call === 'GET /orgs/{org}/invitations') {
                            return $this->routers->router🔀Get🔀Orgs()->listPendingInvitations($params);
                        }
                        ...
                        if ($call === 'GET /orgs/{org}/rulesets') {
                            return $this->routers->router🔀Get🔀Repos()->getOrgRulesets($params);
                        }
                    } elseif ($pathChunks[3] === 'security-advisories') {
                        if ($call === 'GET /orgs/{org}/security-advisories') {
                            return $this->routers->router🔀Get🔀SecurityAdvisories()->listOrgRepositoryAdvisories($params);
                        }
                ...
                if ($pathChunks[2] === '{team_id}') {
                    if ($pathChunks[3] === 'discussions') {
                        ...
                        if ($call === 'GET /teams/{team_id}/invitations') {
                            return $this->routers->router🔀Get🔀Teams()->listPendingInvitationsLegacy($params);
                        }
                    } elseif ($pathChunks[3] === 'members') {
                ...
                if ($pathChunks[2] === 'blocks') {
                    if ($pathChunks[3] === '{username}') {
                        if ($call === 'GET /user/blocks/{username}') {
                            return $this->routers->router🔀Get🔀Users()->checkBlocked($params);
                        }
                    }
                } elseif ($pathChunks[2] === 'codespaces') {
                    if ($pathChunks[3] === 'secrets') {
                        if ($call === 'GET /user/codespaces/secrets') {
                            return $this->routers->router🔀Get🔀Codespaces()->listSecretsForAuthenticatedUser($params);
                        }
                    } elseif ($pathChunks[3] === '{codespace_name}') {
                        if ($call === 'GET /user/codespaces/{codespace_name}') {
                            return $this->routers->router🔀Get🔀Codespaces()->getForAuthenticatedUser($params);
                        }
                    }
                    ...
                    if ($pathChunks[3] === 'stubbed') {
                        if ($call === 'GET /user/marketplace_purchases/stubbed') {
                            return $this->routers->router🔀Get🔀Apps()->listSubscriptionsForAuthenticatedUserStubbed($params);
                        }
                    }
                    ...
                    if ($pathChunks[3] === '{migration_id}') {
                        if ($call === 'GET /user/migrations/{migration_id}') {
                            return $this->routers->router🔀Get🔀Migrations()->getStatusForAuthenticatedUser($params);
                        }
                    }
