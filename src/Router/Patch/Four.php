...
final class Four
{
    ...
    /** @return |array{code:int}|Schema\ProjectColumn|iterable<Schema\Email> */
                ...
                if ($pathChunks[2] === 'codespaces') {
                    if ($pathChunks[3] === '{codespace_name}') {
                        if ($call === 'PATCH /user/codespaces/{codespace_name}') {
                            return $this->routers->router🔀Patch🔀Codespaces()->updateForAuthenticatedUser($params);
                        }
                    }
                } elseif ($pathChunks[2] === 'email') {
                    if ($pathChunks[3] === 'visibility') {
                        if ($call === 'PATCH /user/email/visibility') {
                            return $this->routers->router🔀Patch🔀Users()->setPrimaryEmailVisibilityForAuthenticatedUser($params);
                        }
                    }
