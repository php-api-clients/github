...
final class Eight
{
    ...
    /** @return |Schema\Codespace|array{code:int}|iterable<Schema\Deployment> */
                                ...
                                if ($pathChunks[6] === '{codespace_name}') {
                                    if ($pathChunks[7] === 'stop') {
                                        if ($call === 'POST /orgs/{org}/members/{username}/codespaces/{codespace_name}/stop') {
                                            return $this->routers->router🔀Post🔀Codespaces()->stopInOrganization($params);
                                        }
                                    }
                                }
                            }
                                ...
                                if ($pathChunks[6] === '{run_id}') {
                                    if ($pathChunks[7] === 'approve') {
                                        if ($call === 'POST /repos/{owner}/{repo}/actions/runs/{run_id}/approve') {
                                            return $this->routers->router🔀Post🔀Actions()->approveWorkflowRun($params);
                                        }
