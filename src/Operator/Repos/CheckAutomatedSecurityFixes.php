...
final readonly class CheckAutomatedSecurityFixes
{
    ...
    public function call(string $owner, string $repo): \ApiClients\Client\GitHub\Schema\CheckAutomatedSecurityFixes|array
    {
        ...
        $result    = await($this->browser->request($request->getMethod(), (string) $request->getUri(), $request->withHeader('Authorization', $this->authentication->authHeader())->getHeaders(), (string) $request->getBody())->then(static function (ResponseInterface $response) use ($operation): \ApiClients\Client\GitHub\Schema\CheckAutomatedSecurityFixes|array {
